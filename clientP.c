...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        char server_sequence[10];
        char client_sequence[10];
        char ans;
        if ((numbytes = recv(sockfd, server_sequence, MAXDATASIZE - 1, 0)) == -1) {
            perror("recv");
            exit(1);
        }

        server_sequence[numbytes] = '\0';
        scanf("Enter your answer\n %s", ans);
        client_sequence[len(ans)] = '\0';
        write(sockfd, client_sequence, strlen(client_sequence));
